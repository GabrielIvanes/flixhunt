...
import {mediaToElement, personAggregateToElement, personToElement, providerToElement} from "@/lib/utils";
import {Configuration} from "@/types/tmdb-interfaces";
import {getConfiguration} from "@/lib/tmdb";
import {getTvshowAirDates} from "@/services/tv-shows";
import Separator from "@/components/ui/separator";
import Link from "next/link";
import {Button} from "@/components/ui/button";
import {Bookmark, Clapperboard, Eye, Heart, List, Play, SquarePen} from "lucide-react";
import {Cast, CastAggregateCredit, Crew, CrewAggregateCredit} from "@/types/person-interfaces";
import {getAggregateCast, getAggregateCrew, getCast, getCrew} from "@/services/persons";
...
export default async function Tvshow({params}: { params: Promise<{ tvshowId: string }> }) {
    ...
    const airDate = getTvshowAirDates(tvshow.first_air_date, tvshow.last_air_date, tvshow.status);
    ...
    const cast: CastAggregateCredit[] = getAggregateCast(tvshow.aggregate_credits.cast);
    const crew: CrewAggregateCredit[] = getAggregateCrew(tvshow.aggregate_credits.crew);
    const castElements: ElementInterface[] = cast.map((c) => personAggregateToElement(c, configuration, 175, 175 * 1.5, true, true, true))
    const crewElements: ElementInterface[] = crew.map((c) => personAggregateToElement(c, configuration, 175, 175 * 1.5, true, true, true))
    const recommendationElements: ElementInterface[] = tvshow.recommendations.results.map((recommendation) => mediaToElement(recommendation, recommendation.media_type === "tv" ? "tv-show" : recommendation.media_type, configuration, 175, 175 * 1.5, true, false, false))
                    ...
                    <div className="mx-2 w-full flex flex-col justify-between"
                        ...
                        <div className="flex flex-col flex-1 min-h-0">
                            ...
                            <div className="flex items-center gap-1 mb-2">
                                ...
                                {tvshow.number_of_seasons &&
                                    <div className="flex items-center gap-1">
                                        {airDate && <Separator/>}
                                        <MutedP text={`${tvshow.number_of_seasons}s`}/>
                                    </div>}
                                {tvshow.number_of_episodes &&
                                    <div className="flex items-center gap-1">
                                        {(airDate || tvshow.number_of_seasons) && <Separator/>}
                                        <MutedP text={`${tvshow.number_of_episodes}ep`}/>
                                    </div>}
                                {voteAverage &&
                                    <div className="flex items-center gap-1">
                                        {(airDate || tvshow.number_of_seasons || tvshow.number_of_episodes) &&
                                            <Separator/>}
                                        <MutedP text={`${voteAverage}/10`}/>
                                    </div>}
                                {tvshow.genres.map((genre: Genre, index: number) => (
                                    <div className="flex items-center gap-1" key={genre.id}>
                                        {index === 0 ? (airDate || tvshow.number_of_seasons || tvshow.number_of_episodes || voteAverage) &&
                                            <Separator/> :
                                            <Separator/>}
                            ...
                            <div className="flex items-center">
                                {tvshow.created_by && tvshow.created_by.length > 0 ? tvshow.created_by.map((creator, index: number) => (
                                    <div key={creator.id} className="flex">
                                        <Link href={`/persons/${creator.id}`} passHref><LinkP
                                            text={creator.name}/></Link>
                                        {index != tvshow.created_by.length - 1 && <span>,&nbsp;</span>}
                                    </div>
                            ...
                            <div className="min-h-0 overflow-y-auto pr-2">
                                <P text={tvshow.overview ? tvshow.overview : "There is no overview provided."}/>
                            </div>
