            ...
            <div className="flex flex-1 justify-center gap-x-5 text-base">
                {links.map((link: Link, index: number) => (
                    <Link
                        ...
                        href={link.href}
                        passHref
                        className={`hover:text-card-foreground ${pathname.includes(link.href) ? 'text-card-foreground' : 'text-muted-foreground'} cursor-pointer transition-colors`}
                    >
                        {link.name}
                    </Link>
                ))}
